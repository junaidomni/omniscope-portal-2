...
export async function processIntelligenceData(rawData: IntelligenceData, createdBy?: number) {
  ...
  // Add meeting type as a category if available from intelligence data
  const meetingType = (data.intelligenceData as any)?.meetingType;
  if (meetingType && typeof meetingType === 'string' && meetingType !== 'General') {
    try {
      await db.addCategoryToMeeting(meetingId, meetingType);
      console.log(`[Ingestion] Added meeting type category: ${meetingType}`);
    } catch (e) {
  ...
  if (data.actionItems && data.actionItems.length > 0) {
    for (const item of data.actionItems) {
      ...
      await db.createTask({
        ...
        assignedName: assignedToName || null,
        meetingId: meetingId,
        dueDate: taskDueDate,
